...
def fetch_sample_data(ticker: yf.Ticker, start_time: date, end_time: date, save_file: bool = True) -> pd.DataFrame:
    """Fetch OHLCV data with cache-first behavior and legacy fallback.

    Cache coverage is checked with a small tolerance (5 days) to handle
    weekends and market holidays without forcing unnecessary refetches.
    """
...
def fetch_auxiliary_data(start_date: str = None, end_date: str = None, verbose: bool = False) -> pd.DataFrame:
    """Fetch auxiliary market indicators with cache-first behavior.

    Loaded data is normalized to canonical feature names and a tz-naive index
    to match downstream joins with per-ticker price data.
    """
...
def fetch_data_for_eda(start_time: date, end_time: date) -> Dict[str, pd.DataFrame]:
    """Fetch raw price data for all configured tickers."""
    stocks_data = {}

    for ticker in TICKERS:
        ticker_obj = yf.Ticker(ticker)
        stocks_data[ticker] = fetch_sample_data(ticker_obj, start_time, end_time)
    return stocks_data


def filter_to_model_range(
    data: Dict[str, pd.DataFrame],
    model_start: date = START_DATE,
) -> Dict[str, pd.DataFrame]:
    """Trim each dataframe to the modeling start date and later."""
