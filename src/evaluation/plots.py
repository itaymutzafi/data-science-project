...
def plot_price_vs_returns(df: pd.DataFrame, target_col: str) -> None:
    """Dual-axis plot for raw prices vs. log-returns."""
    set_style()
    fig, axes = plt.subplots(2, 1, sharex=True)

    axes[0].plot(
        df.index, df["Close"], label="Close Price", color=COMPANY_COLORS.get(TICKER, "#1f77b4")
    )
...
def plot_autocorrelation(series: pd.Series, lags: int = 40) -> None:
...
def plot_sentiment_vs_price(df: pd.DataFrame, sentiment_col: str = "sentiment_mean_lag1", days: int | None = None) -> None:
    ...
    ax2 = ax1.twinx()
    ax2.set_ylabel("Sentiment Score (Lagged)", color="tab:orange")
    ax2.plot(plot_data.index, plot_data[sentiment_col], color="tab:orange", linestyle="--", label="Sentiment (Lag 1)", alpha=0.6)
    ax2.tick_params(axis="y", labelcolor="tab:orange")

    title_prefix = df["Ticker"].iloc[0] if "Ticker" in df.columns and not df.empty else "Price"
    plt.title(f"{title_prefix} Stock Price vs. FinBERT Sentiment {title_suffix}")
    fig.tight_layout()
    plt.show()


def plot_sentiment_signal(df: pd.DataFrame, ticker: str | None = None, sentiment_col: str = "sentiment_mean_lag1", days: int | None = None) -> None:
    """Alias for sentiment vs price overlay to match notebook API."""
    plot_sentiment_vs_price(df, sentiment_col=sentiment_col, days=days)
